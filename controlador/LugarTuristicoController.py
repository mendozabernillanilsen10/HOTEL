...
class LugarTuristicoController:
    @classmethod
    def insertar_lugar_turistico(cls, p_nombre, p_descripcion, p_ubicacion, p_foto_url):
        conexion = obtener_conexion()
        with conexion.cursor() as cursor:
            cursor.execute(
                "INSERT INTO lugares_turisticos (nombre, descripcion, ubicacion, foto_url) VALUES (%s, %s, %s, %s)",
                (p_nombre, p_descripcion, p_ubicacion, p_foto_url),
            )
        conexion.commit()
        conexion.close()

    @classmethod
    def actualizar_lugar_turistico(
        cls, p_id, p_nombre, p_descripcion, p_ubicacion, p_foto_url
    ):
        conexion = obtener_conexion()
        with conexion.cursor() as cursor:
            cursor.execute(
    ...
    def obtener_lugar_turistico_por_id(cls, p_id):
        ...
        with conexion.cursor() as cursor:
            cursor.execute(
                "SELECT id, nombre, descripcion, ubicacion, foto_url FROM lugares_turisticos WHERE id = %s",
                (p_id,),
            )
    ...
    def obtener_lugares_turisticos(cls):
        ...
        with conexion.cursor() as cursor:
            cursor.execute(
                "SELECT id, nombre, descripcion, ubicacion, foto_url FROM lugares_turisticos"
            )
